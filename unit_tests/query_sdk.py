...
if qtype == 'all' or qtype == 'profile':
    ...
    try:
        if cmd == "get":
            rv = get_profile(base.cloudSDK_url, base.bearer, base.customer_id,
                             base.command_line_args.object_id)
            print("Profiles for customer %s  (%i pages):" % (base.customer_id, len(rv)))
            #jobj = json.load(ssids)
            for r in rv:
                if brief:
                    for p in r['items']:
                        print("Profile id: %s name: %s  type: %s" %
                              (p['id'], p['name'], p['profileType']))
                else:
        ...
        if cmd == "delete":
            ...
            if delid.isdigit():
                rv = base.cloud.delete_profile(
                    base.cloudSDK_url, base.bearer, base.command_line_args.object_id)
                print("Delete profile for customer %s, id: %s results:" %
                      (base.customer_id, base.command_line_args.object_id))
                print(rv.json())
            else:
                # Query object by name to find its ID
                targets = get_profile(base.cloudSDK_url, base.bearer,
                                      base.customer_id, base.command_line_args.object_id)
                for me in targets:
    ...
    try:
        if cmd == "get":
            ...
            for e in rv:
                if brief:
                    for eq in e['items']:
                        print("Equipment id: %s inventoryId: %s profileId: %s type: %s" %
                              (eq['id'], eq['inventoryId'], eq['profileId'], eq['equipmentType']))
                else:
                    print(json.dumps(e, indent=4, sort_keys=True))
        if cmd == "delete":
            delid = base.command_line_args.object_id
            rv = base.cloud.delete_equipment(
                base.cloudSDK_url, base.bearer, base.command_line_args.object_id)
...
if qtype == 'all' or qtype == 'client-sessions':
    ...
    try:
        rv = base.cloud.get_customer_client_sessions(
            base.cloudSDK_url, base.bearer, base.customer_id)
...
if qtype == 'all' or qtype == 'service-metric':
    ...
    try:
        ...
        rv = base.cloud.get_customer_service_metrics(
            base.cloudSDK_url, base.bearer, base.customer_id, fromTime, toTime)
...
if qtype == 'all' or qtype == 'event':
    ...
    try:
        fromTime = "0"
        toTime = "%i" % (0xFFFFFFFFFFFF)  # something past now, units are msec
        rv = base.cloud.get_customer_system_events(
            base.cloudSDK_url, base.bearer, base.customer_id, fromTime, toTime)
